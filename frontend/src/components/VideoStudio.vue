...
watch(()=>selectedShot.value?.id,()=>{historyPreviewIndex.value=null})
  ...
  <div class="video-editor" v-if="project.shots.length"><VideoShotNavigator :shots="project.shots" :selected="selected" :image-url="imageUrl" :has-warning="shotHasPromptWarning" @select="selected=$event;boundary=1;closeBoundaryEditor()" />
   <div v-for="(shot,i) in project.shots" :key="shot.id" v-show="i===selected" class="video-detail">
    ...
    <section v-if="project.status==='image_review'&&shot.image_status==='completed'" class="storyboard-edit-tools">
     <div class="storyboard-edit-heading"><div><strong>修正这张核心分镜图</strong><p class="muted">提示词修改只在点击重绘时提交；手选参考图最多 3 张，编号按下方顺序传给图像模型。</p><p v-if="!useCurrentReference&&!redrawReferenceIds.length" class="muted">未手选参考图：沿用本镜头原有的人物及其他参考素材（如有）。</p><p v-if="sceneAssetFor(shot)&&useSceneReference" class="muted">场景图自动附在这些参考素材之后，只约束空间布局。</p></div><select v-model="redrawResolution" aria-label="重绘分辨率"><option value="">跟随任务分辨率</option><option value="1k">1K</option><option value="2k">2K</option><option value="4k">4K</option></select></div>
     <div class="storyboard-reference-list">
      <button type="button" :class="{active:useCurrentReference}" :disabled="busy||imageAssetRunning(shot)" @click="toggleCurrentReference"><span v-if="useCurrentReference">图1 · </span>本图作为参考</button>
      <button v-for="asset in redrawReferences" :key="asset.id" type="button" :class="{active:redrawReferenceIds.includes(asset.id)}" :disabled="busy||imageAssetRunning(shot)" @click="toggleRedrawReference(asset.id)"><span v-if="redrawReferenceIds.includes(asset.id)">图{{redrawReferenceNumber(asset.id)}} · </span>{{asset.name}}</button>
      <label class="file-action">＋ 上传参考图<input type="file" accept="image/jpeg,image/png,image/webp" multiple :disabled="busy||imageEditRunning" @change="uploadRedrawReferences"></label>
     </div>
